...
void eval(char *cmdline) 
{
    ...
    //eliminating run between addjob and deletejob
    //also block all signal while addjob
    sigfillset(&mask_all);
...
void do_bgfg(char **argv) 
{
    ...
    if(argv[1][0]=='%'&&is_integer(argv[1]+1)){ 
        //jid
        pid_t jid = atoi(argv[1]+1);
        targetjob = getjobjid(jobs,jid);
        if(targetjob == NULL){ 
...
int is_integer(const char *str) {
    ...
    if (*str == '\0') return 0; // No digits after sign

    while (*str) {
        if (!isdigit(*str)) return 0; // Not a digit
        str++;
    }
